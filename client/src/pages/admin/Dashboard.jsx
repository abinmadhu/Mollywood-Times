      ...
      <div className="mt-10 w-full">
        ...
        <div className="flex flex-wrap gap-5 mt-5">
          {dashboardData.activeShows.map((show, index) => (
            <div key={index} className="w-55 h-full pb-4  overflow-hidden rounded-lg bg-secondary/10 border border-secondary/20 hover:-translate-y-1 transition duration-300">
              ...
              <div className="flex items-end justify-between px-2 ">
                <p className="font-semibold">{currency} {show.showPrice}</p>
                <div className="flex items-center gap-1 text-sm ">
                  <Star className="w-4 h-4 fill-secondary text-secondary"/>
                  <p className="text-gray-300/30">{show.movie.vote_average.toFixed(1)}</p>
                </div>
              </div>
